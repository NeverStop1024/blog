...
因为我们vue项目用的是ts，所以要手动指定`parserOptions.parser`，至于为什么，文章`工具->eslint`中有写到。
```json
parserOptions: {
  ecmaVersion: 'latest',
  sourceType: 'module',
...
* 自定义commitlint规则,执行`yarn add commitlint-config-cz cz-customizable -D`
* 创建`.cz-config.js`
```javascript
module.exports = {
  types: [
...
```
vite创建的项目，`package.json`中指定了`"type": "module"`，说明js文件默认使用ESM模块规范，不支持commonjs规范，但是我们刚刚创建的几个配置文件`.cz-config.js`、`commitlint.config.js`都使用了`module.exports`语法。
...
由于我们改了`.cz-config.js`后缀，cz-customizable找不到配置文件了，我们需要在`package.json`手动加入
```json
  "config": {
    "commitizen": {
      "path": "./node_modules/cz-customizable"
